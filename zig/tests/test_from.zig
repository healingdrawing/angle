...
pub fn test_from(epsilon: f32) !report.MethodResult {
    ...
    for (data_from.cases) |tcase| {
        // Call Zig method
        const zig_angle = angle.from(tcase.in_unit, tcase.value);
        ...
        // Call TS bridge
        const data_str = try floatUtils.vectors_to_string(allocator, .{ @intFromEnum(tcase.in_unit), tcase.value, @intFromEnum(tcase.out_unit) });
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            std.debug.print("✓ Angle.from({any}, {any}){any}() = {any}\n", .{ tcase.in_unit, tcase.value, tcase.out_unit, zig_result });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ Angle.from",
