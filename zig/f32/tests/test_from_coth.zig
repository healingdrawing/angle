...
const data_from_coth = @import("data_from_coth.zig");
const floatUtils = @import("float.zig");
const Angle = @import("../angle.zig").Angle;
const unit = @import("../angle.zig").AngleUnit;
const report = @import("report.zig");

pub fn test_from_coth(comptime T: type, epsilon: T) !report.MethodResult {
    ...
    for (data_from_coth.cases(T)) |tcase| {
        // Call Zig method
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            dp.rawdevlog(.{ "✓ Angle.from_coth(", tcase.in_value, ")", tcase.out_unit, "() = ", zig_result, "\n" });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ Angle.from_coth",
    ...
    return .{
        ...
        .total = data_from_coth.cases(T).len,
        .failed = failed,
    };
}
