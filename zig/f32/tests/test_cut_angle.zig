...
pub fn test_cut_angle(comptime T: type, epsilon: T) !report.MethodResult {
    ...
    for (data_cut.cases(T)) |tcase| {
        ...
        // Call TS bridge
        const data_str = try floatUtils.vectors_to_string(T, allocator, .{ @intFromEnum(tcase.base_unit), tcase.base_value, @intFromEnum(tcase.cut_unit), tcase.cut_value, @intFromEnum(tcase.out_unit) });
