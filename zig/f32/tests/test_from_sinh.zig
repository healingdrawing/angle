...
const data_from_sinh = @import("data_from_sinh.zig");
const floatUtils = @import("float.zig");
const Angle = @import("../angle.zig").Angle;
const unit = @import("../angle.zig").AngleUnit;
const report = @import("report.zig");

pub fn test_from_sinh(comptime T: type, epsilon: T) !report.MethodResult {
    const allocator = std.heap.page_allocator;
    var failed: usize = 0;

    const angle = Angle(T);

    dp.devlog(.{"Running Angle.from_sinh tests..."});

    for (data_from_sinh.cases(T)) |tcase| {
        // Call Zig method
        const zig_angle = angle.from_sinh(tcase.in_value);
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../../ts/terminalcall.ts from_sinh {s}", .{data_str});
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            dp.rawdevlog(.{ "✓ Angle.from_sinh(", tcase.in_value, ")", tcase.out_unit, "() = ", zig_result, "\n" });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ Angle.from_sinh",
    ...
    return .{
        .name = "Angle.from_sinh",
        .total = data_from_sinh.cases(T).len,
        .failed = failed,
    };
}
