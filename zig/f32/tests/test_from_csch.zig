...
const data_from_csch = @import("data_from_csch.zig");
const floatUtils = @import("float.zig");
const Angle = @import("../angle.zig").Angle;
const unit = @import("../angle.zig").AngleUnit;
const report = @import("report.zig");

pub fn test_from_csch(comptime T: type, epsilon: T) !report.MethodResult {
    const allocator = std.heap.page_allocator;
    var failed: usize = 0;

    const angle = Angle(T);

    dp.devlog(.{"Running Angle.from_csch tests..."});

    for (data_from_csch.cases(T)) |tcase| {
        // Call Zig method
        const zig_angle = angle.from_csch(tcase.in_value);
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../../ts/terminalcall.ts from_csch {s}", .{data_str});
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            dp.rawdevlog(.{ "✓ Angle.from_csch(", tcase.in_value, ")", tcase.out_unit, "() = ", zig_result, "\n" });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ Angle.from_csch",
    ...
    return .{
        .name = "Angle.from_csch",
        .total = data_from_csch.cases(T).len,
        .failed = failed,
    };
}
