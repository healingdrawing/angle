...
const data_from_cos = @import("data_from_cos.zig");
const floatUtils = @import("float.zig");
const Angle = @import("../angle.zig").Angle;
const unit = @import("../angle.zig").AngleUnit;
const report = @import("report.zig");

pub fn test_from_cos(comptime T: type, epsilon: T) !report.MethodResult {
    const allocator = std.heap.page_allocator;
    var failed: usize = 0;

    const angle = Angle(T);

    dp.devlog(.{"Running Angle.from_cos tests..."});

    for (data_from_cos.cases(T)) |tcase| {
        // Call Zig method
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../../ts/terminalcall.ts from_cos {s}", .{data_str});
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            std.debug.print("✓ Angle.from_cos({d}){any}() = {any}\n", .{ tcase.in_value, tcase.out_unit, zig_result });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ Angle.from_cos",
    ...
    return .{
        .name = "Angle.from_cos",
        .total = data_from_cos.cases(T).len,
        .failed = failed,
    };
}
