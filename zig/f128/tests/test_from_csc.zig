...
pub fn test_from_csc(epsilon: f128) !report.MethodResult {
    ...
    for (data_from.cases) |tcase| {
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            dp.rawdevlog(.{ "✓ Angle.from_csc(", tcase.in_value, ")", tcase.out_unit, "() = ", zig_result, "\n" });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ Angle.from_csc",
