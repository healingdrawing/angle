...
pub fn test_normalize(epsilon: f128) !report.MethodResult {
    ...
    for (data_normalize.cases) |tcase| {
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../../ts/terminalcall.ts normalize {s}", .{data_str});
