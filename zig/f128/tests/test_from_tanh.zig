...
const data_from_tanh = @import("data_from_tanh.zig");
const floatUtils = @import("float.zig");
const Angle = @import("../angle.zig").Angle;
const unit = @import("../angle.zig").AngleUnit;
const report = @import("report.zig");

pub fn test_from_tanh(comptime T: type, epsilon: T) !report.MethodResult {
    const allocator = std.heap.page_allocator;
    var failed: usize = 0;

    const angle = Angle(T);

    dp.devlog(.{"Running Angle.from_tanh tests..."});

    for (data_from_tanh.cases(T)) |tcase| {
        // Call Zig method
        const zig_angle = angle.from_tanh(tcase.in_value);
        ...
        const cmd = try std.fmt.allocPrint(allocator, "/home/user/.bun/bin/bun ../../ts/terminalcall.ts from_tanh {s}", .{data_str});
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            dp.rawdevlog(.{ "✓ Angle.from_tanh(", tcase.in_value, ")", tcase.out_unit, "() = ", zig_result, "\n" });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ Angle.from_tanh",
    ...
    return .{
        .name = "Angle.from_tanh",
        .total = data_from_tanh.cases(T).len,
        .failed = failed,
    };
}
