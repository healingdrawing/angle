...
const data_from_tan = @import("data_from_tan.zig");
const floatUtils = @import("float.zig");
const Angle = @import("../angle.zig").Angle;
const unit = @import("../angle.zig").AngleUnit;
const report = @import("report.zig");

pub fn test_from_tan(comptime T: type, epsilon: T) !report.MethodResult {
    ...
    for (data_from_tan.cases(T)) |tcase| {
        // Call Zig method
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            dp.rawdevlog(.{ "✓ Angle.from_tan(", tcase.in_value, ")", tcase.out_unit, "() = ", zig_result, "\n" });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ Angle.from_tan",
    ...
    return .{
        .name = "Angle.from_tan",
        .total = data_from_tan.cases(T).len,
        .failed = failed,
    };
}
