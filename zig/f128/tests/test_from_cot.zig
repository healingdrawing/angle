...
const data_from_cot = @import("data_from_cot.zig");
const floatUtils = @import("float.zig");
const Angle = @import("../angle.zig").Angle;
const unit = @import("../angle.zig").AngleUnit;
const report = @import("report.zig");

pub fn test_from_cot(comptime T: type, epsilon: T) !report.MethodResult {
    ...
    for (data_from_cot.cases(T)) |tcase| {
        // Call Zig method
        ...
        if (ok_zig_ts and ok_zig_exp and ok_ts_exp) {
            dp.rawdevlog(.{ "✓ Angle.from_cot(", tcase.in_value, ")", tcase.out_unit, "() = ", zig_result, "\n" });
        } else {
            failed += 1;
            dp.errlog(.{
                "✗ Angle.from_cot",
    ...
    return .{
        .name = "Angle.from_cot",
        .total = data_from_cot.cases(T).len,
        .failed = failed,
    };
}
